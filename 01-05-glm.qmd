...
emmeans(bostyn_model2, 
        specs = "study", 
        type = "response") # transform from logit
```

By adding the `type = "response"` argument, it transforms the estimates from the logit scale to the response scale, i.e., probabilities. These estimates correspond nicely with what we reconstructed from the model coefficients. 
...
There are two ways of modelling this process depending on the link function. The default in the <pkg>ordinal</pkg> package is a logit link, where you model the data using a logistic function. With this approach, you interpret predictor coefficients in log odds to model a shift in probability and you must convert the estimates to an odds ratio or predicted probabilities to aid interpretation. In contrast, you can specify a probit link which models the data as a standard normal or Gaussian distribution. With this approach, you interpret predictor coefficients as changes in standard deviations or z-scores. The probit link function means you can interpret the estimates more like a regular linear regression model with standardised predictors. @gambarota_ordinal_2024 includes a nice explanation of interpreting ordinal regression model depending on the link function and when you would choose one over the other. 
...
In the first segment of the output, its somewhat similar to the other models we have fitted, but there is no intercept at the top. We just have the slope coefficients for each predictor (one in this case), then the threshold coefficients for *k*-1 scale points. 
...
- In the new model, our key predictor of `study` is statistically `r mcq(opts = sample(c(answer = "significant", "non-significant")))`. The two new predictors of `AGE` and `GENDER` are statistically `r mcq(opts = sample(c("significant", answer =  "non-significant")))`.  

- The slope for `study` is still in log odds and negative (unless you flipped the groups). If you take the reciprocal of the exponential slope, the odds ratio for the predictor to 2 decimals is `r fitb(answer = 2.23, num = TRUE, tol = .001)` [lower 95% CI = `r fitb(answer = 1.15, num = TRUE, tol = .001)`, upper 95% CI = `r fitb(answer = 4.31, num = TRUE, tol = .001)`].

- This suggests there are `r fitb(answer = 2.23, num = TRUE, tol = .001)` lower odds of making a consequential decision in the `r mcq(opts = sample(c("real", answer = "hypothetical")))` group compared to the `r mcq(opts = sample(c(answer = "real", "hypothetical")))` group. 

- Rounding to 1 decimal, the estimated probability of making a consequential decision in the real group is `r fitb(answer = 87.4, num = TRUE, tol = .001)`% (95% CI = `r fitb(answer = 81.6, num = TRUE, tol = .001)`, `r fitb(answer = 91.6, num = TRUE, tol = .001)`) and the estimated probability in the hypothetical group is `r fitb(answer = 75.8, num = TRUE, tol = .001)`% (95% CI = `r fitb(answer = 64.3, num = TRUE, tol = .001)`, `r fitb(answer = 84.4, num = TRUE, tol = .001)`).
:::
...
Finally, you can calculate the estimated probability of making a consequentialist decision in each group by using the `emmeans()` function. If you calculated it manually with the other methods, that's fine too. 
