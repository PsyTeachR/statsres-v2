...
For the guided examples, we have two datasets, but we will introduce them in turn. To demonstrate logistic regression, we will use data from X. 

Bostyn et al. studied... Their research question was: X The hypothesis was: X. 

There is one independent variable (`Condition`) to randomly allocate participants into one of two groups: 
...
```{r}
bostyn_data %>% 
  ggplot(aes(x = decision, fill = study)) + 
  geom_bar(position = "dodge") + 
  scale_x_discrete(name = "Decision") + 
  scale_y_continuous(name = "Frequency", 
                     breaks = seq(0, 200, 50), 
                     limits = c(0,200)) + 
  scale_fill_viridis_d(name = "Study", option = "E") +
  theme_minimal()
```
