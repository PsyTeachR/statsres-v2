...
To reinforce this concept, we can visualise the data to show how the comparisons work under a dummy coding scheme. 

```{r echo=FALSE}
james_means <- james_data %>% 
  group_by(Condition) %>% 
  summarise(mean_intrusions = mean(post_intrusions))

james_means %>% 
  ggplot(aes(x = Condition, y = mean_intrusions))  + 
  geom_point(size = 3)  + 
  geom_hline(yintercept = RT_mean, linetype = 2, linewidth = 1, colour = "red") + 
  geom_segment(aes(x = Condition, y = mean_intrusions, yend = RT_mean)) + 
  scale_y_continuous(name = "Mean Number of Intrusive Memories",
                     breaks = seq(0, 6, 1),
                     limits = c(0,6)) +
  scale_x_discrete(name = "Experimental Task", 
                   label = c("Control", "Reactivation + Tetris", "Tetris", "Reactivation")) + 
  theme_classic() + 
  labs(title = "Dummy coding is relative to your choice of reference group.")
```
...
You might see different ways of deviation coding. When you use 0.5/-0.5, you can calculate the effect of the group as 0.5 times the slope (i.e., halving the coefficient), which tells you the difference to the grand mean. For 1/-1. you calculate the effect of the group as 1 times the slope (i.e., it does not change), which tells you the difference to the grand mean. We will typically use 0.5/-0.5 for consistency with other materials, but be aware you might see each approach.
...
::: {.callout-note}
Remember the interpretation of the intercept is what the outcome value is when the predictors are set to 0. The interpretation shifts here as the intercept represents the grand mean across the group. When the predictors are set to 0, the outcome is in the middle of -0.5 and 0.5, so its the mid point or average of all observations. When you do not have interactions, this might not be as useful, but its easier to understand the logic when there are no partial effects.  
:::
...
# deviations to interpret 
# Then recreating the 0.5 deviations as 2 times the mean difference 
james_data %>% 
  group_by(Condition) %>% 
  summarise(grand_mean = grand_mean,
            mean_intrusions = mean(post_intrusions),
            deviations = (grand_mean - mean(post_intrusions)),
            `0.5/-0.5 coded` = (grand_mean - mean(post_intrusions)) * 2)
```

::: {.callout-warning}
We can see the grand mean and deviations align with exactly what we expect. It is trickier than dummy coding, but this is what deviation coding does. Note in the model you do not get the group 2 contrast as that's always the reference group and we are ignoring the sign of the deviation to check its consistency. Also pay attention to calculating 2 times the difference between the grand mean and the group mean, since we used 0.5/-0.5 as the coding scheme. Coefficients represent a 1-unit change in the outcome given the predictor, so in essence this doubles the difference in a 0.5/-0.5 coding scheme. If you want to recalculate group means in relation to the intercept, you must *halve* the coefficients. For example, if wanted to calculate the estimated group mean for control, you would calculate: 

$$intrustions_\mathrm{control} = 3.93 + \dfrac{2.36}{2} = 3.93 + 1.18 = 5.11$$
...
Alternatively, you might find a plot helpful to understand how deviation coding works. 

```{r echo=FALSE}
deviation_data <- james_data %>% 
  group_by(Condition) %>% 
  summarise(mean_intrusions = mean(post_intrusions),
            deviations = (grand_mean - mean(post_intrusions)))
...
deviation_data %>% 
  ggplot(aes(x = Condition, y = mean_intrusions))  + 
  geom_point(size = 3)  + 
  geom_hline(yintercept = grand_mean, linetype = 2, linewidth = 1, colour = "red") + 
  geom_segment(aes(x = Condition, y = mean_intrusions, yend = grand_mean)) + 
  scale_y_continuous(name = "Mean Number of Intrusive Memories",
                     breaks = seq(0, 6, 1),
                     limits = c(0,6)) +
  scale_x_discrete(name = "Experimental Task", 
                   label = c("Control", "Reactivation + Tetris", "Tetris", "Reactivation")) + 
  theme_classic() + 
  labs(title = "Deviation coding is relative to the grand mean of all responses.")
```
...
First, plotting is your friend. Sometimes you can plot your design fine using <pkg>ggplot2</pkg>, but you often miss out on the inferential parts for the confidence intervals around estimates, particularly when there are interactions to explore. So, the <pkg>sjPlot</pkg> package can be useful as it can take regression models and plot the model predictions. To decompose an interaction, it is normally more useful to use the original / dummy-coded variables as the units will be more interpretable for the average reader. 
...
Now we have a continuous predictor in the interaction, we cannot use `emmeans()` to break things down. We need a related function called `emtrends()` (estimated marginal trends) which will tell us the slope between our outcome and continuous predictor, for each level of the categorical moderator. 

To get estimates and their confidence intervals for each slope, we can apply `emtrends()` to the model. Again, it's best to use the dummy-coded version rather than the deviation-coded version, so things are easier to interpret in their original units. 

```{r}
emtrends(interaction_model2_dummy, # model object
         specs = "QWE", # categorical predictor
         var = "MachTot") # continuous predictor
```

We can see here the slope is slightly stronger for low-quality workplaces than high-quality workplaces, but this was not a statistically significant difference in slopes given the interaction. 
...
If you did need to break down a significant interaction, we can use `emtrends()` again, but this time with an extra step. We need to pass the function the moderator break points that we want to use. For simple slopes analysis, this is one standard deviation below the mean, the mean, and one standard deviation above the mean. 

```{r}
# Calculate the mean
mean <- mean(evans_data$LOCTot)

# Calculate one standard deviation below the mean
SD_mean <- mean(evans_data$LOCTot) - sd(evans_data$LOCTot)

# Calculate one standard deviation above the mean
mean_SD <- mean(evans_data$LOCTot) + sd(evans_data$LOCTot)
```

We can then use these three values in a list within the function. This is like the continuous by categorical interaction, but we are using different set levels of the moderator. 

```{r}
emtrends(interaction_model3_dummy, # model object
         specs = "LOCTot", # moderator
         var = "MachTot", # predictor
         at = list(LOCTot = c(SD_mean, mean, mean_SD))) # simple slopes
```

As values of locus of control increase, the relationship between Machiavellianism and unethical behaviour intentions decreases (i.e., the slope decreases). The interaction here was not statistically significant, but this is the process you *would* follow to break it down. 
